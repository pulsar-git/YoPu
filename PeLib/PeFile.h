...
namespace PeLib
{
	...
	**/
	...
	{
//        std::cout << "peHeader().calcNumberOfRvaAndSizes() " << peHeader().calcNumberOfRvaAndSizes() <<std::endl;
//        std::cout << "peHeader().getIddImportRva " << peHeader().getIddImportRva() << std::endl;
//        std::cout << "peHeader().getIddImportSize " << peHeader().getIddImportSize() << std::endl;
 
		if (peHeader().calcNumberOfRvaAndSizes() >= 2
			&& peHeader().getIddImportRva()
			&& peHeader().getIddImportSize())
		{
            //std::cout << "PeFileT<bits>::readImportDirectory rvatooffset=" <<  std::hex << peHeader().rvaToOffset(peHeader().getIddImportRva()) << std::endl;
	...
	**/
	...
	template<int bits>
	int PeFileT<bits>::write(const std::string& strFilename)
	{
           //PeFile::write(strFilename);
//           std::cout << "Writing MZHeader" << std::endl;
           mzHeader().write(strFilename,0);

//           std::cout << "Writing peheader" << std::endl;
           peHeader().makeValid(mzHeader().getAddressOfPeHeader());
           peHeader().write(strFilename,mzHeader().getAddressOfPeHeader());
           
//           std::cout << "Writing sections" << std::endl;
           peHeader().writeSections(strFilename);
          
           
//           std::cout << "Writing section data" << std::endl;
           peHeader().writeSectionsData(strFilename);

           unsigned uiImpDir = peHeader().getIddImportRva();
//           std::cout << "Writing impDir" << std::endl;
           impDir().write(strFilename, peHeader().rvaToOffset(uiImpDir), uiImpDir);
