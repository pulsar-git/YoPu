	...
	**/
	...
	class ImportDirectory
	{
		...
		public:
		  ...
		  dword getFunctionOrd(dword dwFilenr, dword dwFuncnr, currdir cdDir) const;
...
//		  word getFunctionHint(const std::string& strFilename, const std::string& strFuncname, currdir cdDir) const throw (PeLibException);
	...
	**/
	template<int bits>
	void ImportDirectory<bits>::clear()
	{
        m_vOldiid.clear();
        m_vNewiid.clear();
    }
...
/**
	...
	**/
	template<int bits>
	dword ImportDirectory<bits>::getFunctionOrd(dword dwFilenr, dword dwFuncnr, currdir cdDir) const
	{
        int ordinal =0;
		...
		if (cdDir == OLDDIR)
		{
			...
			{
				ordinal = m_vOldiid[dwFilenr].originalfirstthunk[dwFuncnr].itd.Ordinal;
			}
			else
			{
				ordinal = m_vOldiid[dwFilenr].firstthunk[dwFuncnr].itd.Ordinal;
			}
		}
		else
		{
			...
			{
				ordinal = m_vNewiid[dwFilenr].originalfirstthunk[dwFuncnr].itd.Ordinal;
			}
			else
			{
				ordinal = m_vNewiid[dwFilenr].firstthunk[dwFuncnr].itd.Ordinal;
			}
		}
		
		return (ordinal & ~(0xFF000000));
	}
	/**
	* Get the name of an imported function.
	* @param dwFilenr Identifies which file should be checked.
	...
	**/
	...
	int ImportDirectory<bits>::read(const std::string& strFilename, unsigned int uiOffset, unsigned int uiSize, const PeHeaderT<bits>& pehHeader)
	{
		...
		{
           std::cout << "Attention, la section contenant l'IAT a problablement déjà été modifiée!!" << std::endl;            
        }
...
//		}
	...
	**/
	...
	int ImportDirectory<bits>::write(const std::string& strFilename, unsigned int uiOffset, unsigned int uiRva)
	{
		...
		if(uiOffset == 0xFFFFFFFF)
        {
          //Oula y a une couille dans le potage la!
          std::cout << "ImpDir -> uiOffset = [0xFFFFFFFF] || Ca sent le paté!" << std::endl;
          return ERROR_OPENING_FILE;
        }
