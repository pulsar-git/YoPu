	...
	**/
	...
	class PeHeaderT : public PeHeader
	{
		...
		protected:
		  ...
		  std::vector< std::vector<char> > m_sectionData; ///Store the sections data!
...
// image directory getters
          ...
          dword setIddBoundImportRva(dword number); // EXPORT
...
// image directory getters
		  ...
		  /// adjust the size of raw data in the section, looking at the size of section_data
		  void adjustSizeOfRawData(word wSectionnr); // EXPORT
		  ...
		  void setCharacteristics(word uiSectionnr, dword dwValue); // EXPORT
  		  ...
  		  /// Read sections data
		  const int readSectionsData(const std::string& strFilename);
		  const int writeSectionsData(const std::string& strFilename);
		  
  		  /// Accessor function for the sections data.
		  const std::vector< std::vector<char> >& sectionsData() const;
		  /// Accessor function for the sections data.
		  std::vector< std::vector<char> >& sectionsData();	  
		  
		  /// Fix pointers to raw data
		  int fixPointerToRawData();
	};
...
//		}
		...
		// TODO: Read Sections Data
        ///m_sectionsData = readSectionsData();
...
//		obBuffer << m_inthHeader.OptionalHeader.BaseOfData;
	...
	**/
	...
	int PeHeaderT<x>::writeSections(const std::string& strFilename) const
	{
		...
		{
            /*
              Georges Gagnerot 3 mai 2006:
                      On zap le test, en effet si l'on veut ecire les données, tester si
                      la taille est inférieur n'est pas vraiment utile! Ca empeche meme
                      d'écrire actuellement dans le fichier dans le cas d'un nouveau
            */
	...
	**/
	...
	template<int x>
	dword PeHeaderT<x>::setIddBoundImportRva(dword number)
	{
		return m_inthHeader.dataDirectories[PELIB_IMAGE_DIRECTORY_ENTRY_BOUND_IMPORT].VirtualAddress=number;
	}
	...
	**/
	...
	void PeHeaderT<x>::setSizeOfRawData(word wSectionnr, dword dwValue)
	{
		...
		if(m_sectionData.size() > wSectionnr)
		   m_sectionData[wSectionnr].resize(dwValue);       
	}
	...
	**/
	template<int x>
	void PeHeaderT<x>::adjustSizeOfRawData(word wSectionnr)
	{
		if(m_sectionData.size() > wSectionnr)
		{
		   m_vIsh[wSectionnr].SizeOfRawData = m_sectionData[wSectionnr].size();
        }        
	...
	**/
	...
	template<int bits>
    const int PeHeaderT<bits>::readSectionsData(const std::string& strFilename)
    {
        m_sectionData.clear();
        ...
        {
            std::vector<char> section(getSizeOfRawData(i));
            ifFile.seekg(getPointerToRawData(i), std::ios::beg);
            ifFile.read(reinterpret_cast<char*>(&section[0]), static_cast<unsigned int>(getSizeOfRawData(i ) ));
            m_sectionData.push_back(section);
        }
		return NO_ERROR;
    }
    
    template<int bits>
    const int PeHeaderT<bits>::writeSectionsData(const std::string& strFilename)
    {
        std::fstream ofFile(strFilename.c_str(), std::ios_base::in);
        ...
        for (unsigned int i=0;i<getNumberOfSections();i++)
        {
            /// On se déplace au début de la section
            ofFile.seekp(getPointerToRawData(i), std::ios::beg);
            
            std::vector<char> *mSection = & (m_sectionData[i]);

			//Only if size > 0!!
			if(mSection->size()>0)
				ofFile.write(reinterpret_cast<const char*>(&(*mSection)[0]), static_cast<unsigned int>(mSection->size()));
        }
    ...
    template<int bits>
    const std::vector< std::vector<char> >& PeHeaderT<bits>::sectionsData() const
	{
       return m_sectionData;
    }
 
    template<int bits>
    std::vector< std::vector<char> >& PeHeaderT<bits>::sectionsData()
    {
        return m_sectionData;               
    }
    
    template<int bits>
    int PeHeaderT<bits>::fixPointerToRawData()
    {
         //
         // Recalculate pointers....
         //
         int cur_prt=getPointerToRawData(0);
         for(int i=0;i<calcNumberOfSections();i++)
         {
                 setPointerToRawData(i,cur_prt);
                 cur_prt+=PeLib::alignOffset(getSizeOfRawData(i), getFileAlignment());
         }
		 return cur_prt;
     }
}
